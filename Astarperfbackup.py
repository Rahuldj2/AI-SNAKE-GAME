...
def move_towards_food(snake, food):
    ...
    if (head_x + UNIT_SIZE, head_y) not in snake[1:]:
        ...
        future_score_increase = int(new_distance <= distance_to_food)
        score_weighted_distance = new_distance + future_score_increase
        possible_moves.append(("right", score_weighted_distance))
    ...
    if (head_x - UNIT_SIZE, head_y) not in snake[1:]:
        ...
        future_score_increase = int(new_distance <= distance_to_food)
        score_weighted_distance = new_distance + future_score_increase
        possible_moves.append(("left", score_weighted_distance))
    ...
    if (head_x, head_y - UNIT_SIZE) not in snake[1:]:
        ...
        future_score_increase = int(new_distance <= distance_to_food)
        score_weighted_distance = new_distance + future_score_increase
        possible_moves.append(("up", score_weighted_distance))
    ...
    if (head_x, head_y + UNIT_SIZE) not in snake[1:]:
        ...
        future_score_increase = int(new_distance <= distance_to_food)
        score_weighted_distance = new_distance + future_score_increase
        possible_moves.append(("down", score_weighted_distance))

    # Sort the possible moves by the weighted sum of distance to food and potential future score increase
    if (possible_moves == []):
        return None
