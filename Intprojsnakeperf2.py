...
def random_food(snake, occupied):
    while True:
...
# Set up a set to store occupied grid squares by the snake's body
occupied_squares = set(snake)
...
# Snake food
food = random_food(snake, occupied_squares)
...
def move_towards_food(snake, food):
    ...
    if (head_x + UNIT_SIZE, head_y) not in occupied_squares:
    ...
    if (head_x, head_y - UNIT_SIZE) not in occupied_squares:
...
while True:
    ...
    # Update the set of occupied grid squares
    occupied_squares.add(new_head)
    ...
    if snake[0] == food:
        ...
        # If not collided with food, remove the tail and its corresponding occupied square
        tail = snake.pop()
        occupied_squares.remove(tail)
